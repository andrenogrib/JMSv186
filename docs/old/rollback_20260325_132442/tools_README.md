...
- `../../packet_trace_live.bat`
  - tails the current session `logs/PacketTrace_<timestamp>.log` in a separate console so packet traffic can be watched without `RirePE`
...
- `logs/PacketTrace_<YYYYMMDD_HHMMSS>.log`

The server console output is also mirrored to:

- `logs/Server_<region>_<version>_<subversion>_<YYYYMMDD_HHMMSS>.log`
